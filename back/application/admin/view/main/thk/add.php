...
// $sql = "SELECT id,ordernum,model,figure,tp,code FROM thk where code in (" . implode(',', $groupData) . ")";
//将退货库查询到的序列号数据添加到成品库中    
$insert = "INSERT INTO cpk(id,ordernum,model,figure,tp,code,pd) SELECT id,ordernum,model,figure,tp,code,pd FROM thk where code in (" . implode(',', $groupData) . ");";
$insert .="UPDATE cpk SET date='$date',glue='$glue' WHERE code in (" . implode(',', $groupData) . "); ";
$insert .="DELETE FROM thk where code in (" . implode(',', $groupData) . ")";
