...
from cpck where status='0' group by ordernum";
}
elseif ($tmp==$sxpl) {
   $sql = "select ordernum,model,figure,tp,count(model) as cnt
from cpck where status='0' group by model order by cnt DESC";
}
else{
      $sql = "select ordernum,model,figure,tp,count(model) as cnt
from cpck where status='0' group by model order by cnt ASC";
}
...
if(! $retval )
{
  ...
  // if (count($retval)>1) {
  //   echo count($retval);
  //此处需要解决，成品数据库中没有条码，不能查询问题。

echo '<h2 class="text-center">成品库存统计<h2>';
