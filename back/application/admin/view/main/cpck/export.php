...
//设置标题行
$sheet->setTitle('出库名细表');
//设置表头
$sheet->setCellValueByColumnAndRow(1, 1, '出库商品入库名细表');
...
$sheet->setCellValueByColumnAndRow(7, 2, '生产日期');
$sheet->setCellValueByColumnAndRow(8, 2, '胶号');
$sheet->setCellValueByColumnAndRow(9, 2, '订单编号');
$sheet->setCellValueByColumnAndRow(10, 2, '代理商名称');
$sheet->setCellValueByColumnAndRow(11, 2, '发货日期');
...
$sheet->getStyle('A2:K2')->applyFromArray($styleArray)->getFont()->setSize(14);
$spreadsheet->getActiveSheet()->getDefaultColumnDimension()->setWidth(20);
 ...
 $result=array_pop($groupData);
    ...
    $sql = "SELECT id,ordernum,model,figure,tp,code,pd,date,glue,ddnum,fdate,agent FROM cpck where code in (" . implode(',', $groupData) . ")";    
    ...
    for ($i=0; $i < $len; $i++) { 
        ...
        $sheet->setCellValueByColumnAndRow(7, $j, $row[$i]['date'] );
        $sheet->setCellValueByColumnAndRow(8, $j, $row[$i]['glue'] );
        $sheet->setCellValueByColumnAndRow(9, $j, $row[$i]['ddnum'] );
        $sheet->setCellValueByColumnAndRow(10, $j, $row[$i]['agent'] );
        $sheet->setCellValueByColumnAndRow(11, $j, $row[$i]['fdate'] );
    ...
    $total_rows = $len + 2;
    //添加所有边框/居中
    $sheet->getStyle('A1:K'.$total_rows)->applyFromArray($styleArrayBody);
