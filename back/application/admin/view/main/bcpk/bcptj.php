...
from bcpk where status='0' group by ordernum";
}
elseif ($tmp==$sxpl) {
   $sql = "select ordernum,model,figure,tp,count(model) as cnt
from bcpk where status='0' group by model order by cnt DESC";
}
else{
      $sql = "select ordernum,model,figure,tp,count(model) as cnt
from bcpk where status='0' group by model order by cnt ASC";
}
