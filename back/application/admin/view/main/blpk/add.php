...
// $sql = "SELECT id,ordernum,model,figure,tp,code FROM fpk where code in (" . implode(',', $groupData) . ")";
//将废品库查询到的序列号数据添加到成品库中    
$insert = "INSERT INTO cpk(id,ordernum,model,figure,tp,code,pd) SELECT id,ordernum,model,figure,tp,code,pd FROM fpk where code in (" . implode(',', $groupData) . ");";
$insert .="UPDATE cpk SET date='$date',glue='$glue' WHERE code in (" . implode(',', $groupData) . "); ";
$insert .="DELETE FROM fpk where code in (" . implode(',', $groupData) . ")";
